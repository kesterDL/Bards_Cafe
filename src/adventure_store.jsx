...
import { getFirestore, collection, doc, setDoc, getDoc, getDocs, onSnapshot, query, where, orderBy, limit, startAfter, endBefore, limitToLast } from 'firebase/firestore';
import { ChevronDown, ChevronUp, Search, ShoppingCart, Trash2, PlusCircle, MinusCircle, User, ShieldCheck, BookOpen, Home, Package, Filter, X } from 'lucide-react';

// --- Firebase Configuration ---
// IMPORTANT: Replace with your actual Firebase config
const firebaseConfig = {
  apiKey: "YOUR_API_KEY", // Replace if you have one, otherwise leave for Canvas to fill
...
const a_app_id = typeof __app_id !== 'undefined' ? __app_id : 'dnd-adventure-store-dev';
const a_firebase_config_str = typeof __firebase_config !== 'undefined' ? __firebase_config : JSON.stringify(firebaseConfig);
...
// --- Mock Data (Used if Firestore is empty or for initial dev) ---
...
const THEMES = ['All', 'Undead', 'Aquatic', 'Dragon', 'Fey', 'Steampunk', 'Mystery', 'Intrigue', 'Horror'];
const LEVEL_RANGES = ['All', '1-3', '3-5', '5-7', '8-10', '10+'];
...
const AdventureCard = ({ adventure, navigateTo, setSelectedAdventure, addToCart }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-xl overflow-hidden transform hover:scale-105 transition-transform duration-300 flex flex-col h-full">
      <img 
        ...
        onError={(e) => { e.target.onerror = null; e.target.src=`https://placehold.co/600x400/333/ccc?text=Image+Error`; }}
      />
...
const HomePage = ({ navigateTo, adventures, setSelectedAdventure, addToCart }) => {
  const featuredAdventures = adventures.slice(0, 3); // Show first 3 as featured
...
const AdventuresPage = ({ navigateTo, allAdventures, setSelectedAdventure, addToCart }) => {
  ...
  const filteredAdventures = allAdventures.filter(adv => {
    ...
    if (selectedLevel !== 'All') {
        ...
        const maxLvl = maxLvlStr ? parseInt(maxLvlStr) : Infinity; // For "10+"
...
const AdventureDetailPage = ({ adventure, addToCart, navigateTo }) => {
        ...
        <div className="md:flex">
          <div className="md:w-1/2">
            <img 
                ...
                onError={(e) => { e.target.onerror = null; e.target.src=`https://placehold.co/800x600/333/ccc?text=Image+Error`; }}
            />
          </div>
...
const CartPage = ({ cartItems, removeFromCart, updateQuantityInCart, navigateTo, showMessage }) => {
  ...
  const handleRemove = (adventureId) => {
          ...
          <div key={item.id} className="flex flex-col sm:flex-row items-center justify-between border-b border-slate-200 py-4 last:border-b-0">
            <div className="flex items-center mb-4 sm:mb-0 w-full sm:w-2/5">
              <img 
                ...
                onError={(e) => { e.target.onerror = null; e.target.src=`https://placehold.co/100x80/333/ccc?text=Error`; }}
              />
              <div>
...
const CheckoutPage = ({ cartItems, handleCheckout, navigateTo, showMessage }) => {
  ...
  if (cartItems.length === 0 && subtotal === 0) { // Check subtotal too as cartItems might clear before navigation finishes
    return (
        <div className="container mx-auto px-4 py-12 text-center">
...
function App() {
  ...
  const [message, setMessage] = useState({ text: null, type: null }); // For modal messages
  ...
  useEffect(() => {
    ...
    // Initial sign-in attempt for custom token or anonymous
    const attemptInitialSignIn = async () => {
        try {
            // Check for __initial_auth_token if running in Canvas environment
            ...
            if (!auth.currentUser) { // Fallback to anonymous if custom token fails and no user
                try {
                    await signInAnonymously(auth);
                } catch (anonError) {
            ...
            setIsAuthReady(true); // Still ready, even if auth failed, to allow app to proceed
        }
    };
  ...
  // Firestore: Fetch adventures and user cart once auth is ready
  useEffect(() => {
    if (!isAuthReady) return; // Wait for auth to be ready

    // Fetch adventures
    const adventuresColRef = collection(db, `artifacts/${a_app_id}/public/data/adventures`);
    const unsubscribeAdventures = onSnapshot(query(adventuresColRef), (snapshot) => {
        let fetchedAdventures = [];
        snapshot.forEach(doc => {
            fetchedAdventures.push({ id: doc.id, ...doc.data() });
        });
        if (fetchedAdventures.length > 0) {
            ...
            // No adventures in Firestore, use mock data and try to upload them
            ...
            MOCK_ADVENTURES.forEach(async (adv) => {
                try {
                    // Use adv.id if it's a string, otherwise Firestore generates one
                    const docRef = typeof adv.id === 'string' ? doc(adventuresColRef, adv.id) : doc(adventuresColRef);
                    await setDoc(docRef, adv); // adv already contains id if it was predefined
        ...
        setAdventures(MOCK_ADVENTURES); // Fallback to mock data on error
    });

    // Fetch cart if user is logged in
    let unsubscribeCart = () => {};
    if (userId) {
        const cartDocRef = doc(db, `artifacts/${a_app_id}/users/${userId}/carts/activeCart`);
        unsubscribeCart = onSnapshot(cartDocRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                setCartItems([]); // No active cart found
        ...
        // User not logged in, clear cart (or load from local storage if implemented)
        setCartItems([]);
    }
  ...
  }, [isAuthReady, userId]); // Re-run if auth state or user ID changes
  ...
  const updateFirestoreCart = async (newCartItems) => {
    if (!userId || !isAuthReady) {
        ...
        // Optionally, save to localStorage here as a fallback for anonymous users before they fully "log in"
        // localStorage.setItem('dndGuestCart', JSON.stringify(newCartItems));
        return;
    }
    try {
      const cartDocRef = doc(db, `artifacts/${a_app_id}/users/${userId}/carts/activeCart`);
  ...
  const addToCart = (adventure, quantity) => {
  ...
  const removeFromCart = (adventureId) => {
    setCartItems(prevItems => {
        const newItems = prevItems.filter(item => item.id !== adventureId);
        updateFirestoreCart(newItems);
        return newItems;
    });
    // Message shown by CartPage itself
  };

  const updateQuantityInCart = (adventureId, newQuantity) => {
  ...
  const handleCheckout = async (formData) => {
    ...
    // In a real app: process payment, save order to Firestore, etc.
    // For this mock:
    
    // Try to save order to Firestore (publicly for this demo, usually private or admin)
    if (isAuthReady && userId) {
        try {
            const ordersColRef = collection(db, `artifacts/${a_app_id}/public/data/orders`);
  ...
  // --- Render Page ---
  const renderPage = () => {
    if (!isAuthReady && adventures.length === 0) { // Show loading only if auth isn't ready AND adventures aren't loaded (first load)
